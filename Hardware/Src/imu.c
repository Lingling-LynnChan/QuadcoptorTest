...
//   static const float KpDef = 0.8f;
//   static const float KiDef = 0.0003f;
//   static Quaternion NumQ = {1.f, 0.f, 0.f, 0.f};
//   GW_Vector3f Gravity, Accel, Gyro, AccelGravity;
//   float HalfDt = dt * 0.5f;
//   float NormQuat;
//   // 提取等效旋转矩阵中的重力分量
//   Gravity.X = 2.f * (NumQ.Q1 * NumQ.Q3 - NumQ.Q0 * NumQ.Q2);
//   Gravity.Y = 2.f * (NumQ.Q0 * NumQ.Q1 + NumQ.Q2 * NumQ.Q3);
//   Gravity.Z = NumQ.Q0 * NumQ.Q0 - NumQ.Q1 * NumQ.Q1 - NumQ.Q2 * NumQ.Q2 +
//               NumQ.Q3 * NumQ.Q3;
//   // 加速度归一化
//   NormQuat = GWM_RSqrt(                  //
...
//            GyroIntegralError.Z;
//   // 一阶龙格库塔法更新四元数
//   NumQ.Q0 += (-NumQ.Q1 * Gyro.X - NumQ.Q2 * Gyro.Y - NumQ.Q3 * Gyro.Z) * HalfDt;
//   NumQ.Q1 += (NumQ.Q0 * Gyro.X - NumQ.Q3 * Gyro.Y + NumQ.Q2 * Gyro.Z) * HalfDt;
//   NumQ.Q2 += (NumQ.Q3 * Gyro.X + NumQ.Q0 * Gyro.Y - NumQ.Q1 * Gyro.Z) * HalfDt;
//   NumQ.Q3 += (-NumQ.Q2 * Gyro.X + NumQ.Q1 * Gyro.Y + NumQ.Q0 * Gyro.Z) * HalfDt;
//   // 四元数归一化
//   NormQuat = GWM_RSqrt(      //
//       GWM_Square(NumQ.Q0) +  //
//       GWM_Square(NumQ.Q1) +  //
...
//   Angle->Yaw =
//       GWM_Arctan2(                                                        //
//           2 * NumQ.Q1 * NumQ.Q2 + 2 * NumQ.Q0 * NumQ.Q3,                  //
//           1.f - 2.f * GWM_Square(NumQ.Q2) - 2.f * GWM_Square(NumQ.Q3)) *  //
//       IMU_RTA;                                              // 偏航角
