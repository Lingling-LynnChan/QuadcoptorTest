...
static const uint16_t GW_Get_Angle_Ms = 1000.f / MPU6050_RATE_HZ;
// static const float GW_Delta_Time = GW_Get_Angle_Ms / 1000.f;

void GW_Task_1ms() {
  // TODO: 定时器
  static uint16_t AngleMsCnt = 0;
  static uint16_t t10ms = 0;
  if (++AngleMsCnt >= GW_Get_Angle_Ms) {
    AngleMsCnt = 0;
    // IMU_Get_Angle(GW_Delta_Time);
  }
  if (++t10ms >= 10) {
    t10ms = 0;
    float pitch = 0, roll = 0, yaw = 0;
    if (ICM20608_Read(&pitch, &roll, &yaw) != HAL_OK) {
      return;
    }
    int32_t integer;
    uint32_t decimal;
    GWM_Ftoiu(roll, &integer, &decimal);
    printf("Roll: %ld.%06lu, ", integer, decimal);
    GWM_Ftoiu(pitch, &integer, &decimal);
    printf("Pitch: %ld.%06lu, ", integer, decimal);
    GWM_Ftoiu(yaw, &integer, &decimal);
    printf("Yaw: %ld.%06lu\n", integer, decimal);
  }
}
...
//       }
//       // 内环测量值
//       GWS_PID.RateX.Measured = GWS_MPU6050.Gyro.X * IMU_GYRO_G;  //
//       X轴角速度r/s GWS_PID.RateY.Measured = GWS_MPU6050.Gyro.Y * IMU_GYRO_G;
//       // Y轴角速度r/s GWS_PID.RateZ.Measured = GWS_MPU6050.Gyro.Z *
//       IMU_GYRO_G;  // Z轴角速度r/s
//       // 外环测量值
